...
const FuturePrediction = () => {
  ...
  const isAnyAnswerGenerating = Object.values(
  ...
  useEffect(() => {
    const fixPendingToFailed = async () => {
      ...
      pendingEntries.forEach(([key]) => {
        updates[`futurePrediction.${key}.status`] = "failed";
      });
      ...
      try {
        ...
        // ✅ Update local state
        // setUserData((prev) => {
        //   const updated = { ...prev.futurePrediction };
  ...
  const handleAskQuestion = async (q) => {
    try {
      ...
      const allAttempts = Object.entries(user?.futurePrediction || {})
      ...
      // ❌ Credit check
      if (!user.credits || user.credits < questionCreditCost) {
        Swal.fire({
      ...
      const birthDetails = [
        {
          ...
          time: user.birthTime,
          place: {
            name: user.locationData?.address,
            latitude: user.locationData?.lat,
            longitude: user.locationData?.lng,
            timezone: tzLookup(user.locationData?.lat, user.locationData?.lng),
          },
      ...
      const res = await fetch(
        "https://us-central1-steer-u.cloudfunctions.net/analyzeAstro",
        {
          ...
          body: JSON.stringify({
            ...
            question_category: categoryMap[q.category] || "A",
          }),
          signal,
        },
      );
      ...
      await updateDoc(userRef, {
        [`futurePrediction.${attemptId}`]: {
          ...
          status: "success",
          category: q.category,
          creditsUsed: questionCreditCost,
          createdAt: new Date(),
        },
      });
    ...
    <div className="font-poppins mt-10">
      <div className="border border-primary m-3 md:m-6 rounded-2xl shadow-xl relative overflow-hidden bg-gradient-to-br from-orange-50 via-white to-orange-100 py-16 md:py-20">
        ...
        <div className="relative z-10 flex flex-col gap-6 px-4 md:px-6 justify-center items-center max-w-5xl mx-auto">
          ...
          <h1 className="text-2xl md:text-4xl lg:text-4xl font-bold text-black text-center p-3 animate-slide-up">
      ...
      <section className="px-4 md:px-8 pb-16  mx-auto">
        {Array.isArray(questions) && selectedCredit !== "customizedQuestion" ? (
          <>
            {/* show count */}
            ...
            <div className="flex flex-col gap-3">
              {questions.map((q, index) => (
                <QuestionAccordion
                  ...
                  userData={userData}
                  questionCreditCost={questionCreditCost}
